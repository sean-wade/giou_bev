...
inline int compare_by_xy(Point p1, Point p2){
    return p1.x<p2.x || (p1.x==p2.x && p1.y<p2.y);
}
 
inline int compare_by_angleP0(Point p1, Point p2, Point p0){
    int c1=cross(p1, p2, p0);  //tp,p1),c2=cross(p[0],tp,p2);
    if(c1==0){
...
inline float convex_hull_area(Point *all_points){
    int point_num = 8;

    std::sort(all_points, all_points + point_num, compare_by_xy);

    // for (int j = 0; j < 8; j++){
    //     std::cout << all_points[j].x << ","<<all_points[j].y << "    ";
    // }
    
    // sort the points by angle to p0
    Point temp;
    for (int j = 0; j < 8; j++){
        for (int i = 0; i < 7; i++){
            // if (compare_by_angleP0(all_points[i], all_points[i + 1], all_points[0])){
            if (point_cmp(all_points[i], all_points[i + 1], all_points[0])){
                temp = all_points[i];
    ...
    int top=0;
    Point convex_ans[point_num];
    convex_ans[top++]=all_points[0];
    convex_ans[top++]=all_points[1];

    for(int i=2; i<point_num; i++)
    {
        if(cross(all_points[i], convex_ans[top-2], convex_ans[top-1])>0) {
            convex_ans[top++] = all_points[i];
        }  
        else 
        {
            top--;  
            while(top >= 2 && cross(all_points[i], convex_ans[top-2], convex_ans[top-1]) <= 0)
            {
                top--;
            }
    ...
    // for (int j = 0; j < top; j++){
    //     std::cout << convex_ans[j].x << ","<<convex_ans[j].y << "    ";
    // }
    // std::cout << std::endl;
    ...
    {
        area += cross(convex_ans[k] - convex_ans[0], convex_ans[k + 1] - convex_ans[0]);
    }
    return fabs(area) / 2.0;
}


inline float giou_bev(const float *box_a, const float *box_b){
    // params: box_a (7) [x, y, z, dx, dy, dz, heading]
    // params: box_b (7) [x, y, z, dx, dy, dz, heading]
...
//    float b_x1 = box_b[0], b_y1 = box_b[1], b_x2 = box_b[2], b_y2 = box_b[3], b_angle = box_b[4];
    ...
    Point all_box_corners[8];
    for (int k = 0; k < 4; k++){
        rotate_around_center(center_a, a_angle_cos, a_angle_sin, box_a_corners[k]);
        rotate_around_center(center_b, b_angle_cos, b_angle_sin, box_b_corners[k]);

        all_box_corners[k] = box_a_corners[k];
        all_box_corners[k+4] = box_b_corners[k];
    }
    ...
    float s_union = sa + sb - s_overlap;
    
    float s_union_convex = convex_hull_area(all_box_corners);

    float iou = s_overlap / fmaxf(s_union, EPS);
    float giou = iou - ((s_union_convex - s_union) / fmaxf(s_union_convex, EPS));

    // std::cout << "iou="<<iou<<", s_union_convex="<<s_union_convex<<", s_union="<<s_union<<", s_overlap="<<s_overlap<<std::endl;

    return giou;
}


int boxes_giou_bev_cpu(at::Tensor boxes_a_tensor, at::Tensor boxes_b_tensor, at::Tensor ans_giou_tensor){
    // params boxes_a_tensor: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b_tensor: (M, 7) [x, y, z, dx, dy, dz, heading]
    // params ans_giou_tensor: (N, M)
    ...
    const float *boxes_a = boxes_a_tensor.data<float>();
    const float *boxes_b = boxes_b_tensor.data<float>();
    float *ans_giou = ans_giou_tensor.data<float>();

    for (int i = 0; i < num_boxes_a; i++){
        for (int j = 0; j < num_boxes_b; j++){
            ans_giou[i * num_boxes_b + j] = giou_bev(boxes_a + i * 7, boxes_b + j * 7);
        }
    }
    return 1;
}
